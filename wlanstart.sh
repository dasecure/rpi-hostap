...
# Default values
true ${SUBNET:=192.168.8.0}
true ${AP_ADDR:=192.168.8.1}
true ${PRI_DNS:=8.8.8.8}
true ${SEC_DNS:=8.8.4.4}
true ${SSID:=Room555}
true ${CHANNEL:=11}
true ${WPA_PASSPHRASE:=4dwinner123@}
true ${HW_MODE:=g}
# ooi additions
true ${OUTGOINGS:=eth0}
...
if [ "${OUTGOINGS}" ] ; then
   ...
   do
      ...
      iptables-nft -t nat -C POSTROUTING -o ${int} -j MASQUERADE || iptables-nft -t nat -A POSTROUTING -o ${int} -j MASQUERADE
      iptables-nft -C FORWARD -i ${int} -o ${INTERFACE} -m state --state RELATED,ESTABLISHED -j ACCEPT || iptables-nft -A FORWARD -i ${int} -o ${INTERFACE} -m state --state RELATED,ESTABLISHED -j ACCEPT
      iptables-nft -C FORWARD -i ${INTERFACE} -o ${int} -j ACCEPT || iptables-nft -A FORWARD -i ${INTERFACE} -o ${int} -j ACCEPT
...
if [ "${OUTGOINGS}" ] ; then
   ...
   do
      ...
      iptables-nft -t nat -C POSTROUTING -o ${int} -j MASQUERADE && iptables-nft -t nat -D POSTROUTING -o ${int} -j MASQUERADE
      iptables-nft -C FORWARD -i ${int} -o ${INTERFACE} -m state --state RELATED,ESTABLISHED -j ACCEPT && iptables-nft -D FORWARD -i ${int} -o ${INTERFACE} -m state --state RELATED,ESTABLISHED -j ACCEPT
      iptables-nft -C FORWARD -i ${INTERFACE} -o ${int} -j ACCEPT && iptables-nft -D FORWARD -i ${INTERFACE} -o ${int} -j ACCEPT
   done
else
   echo "Setting iptables for outgoing traffics on all interfaces..."
